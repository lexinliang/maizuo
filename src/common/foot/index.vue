<template>
     <footer class="nav-bar">
        <router-link v-for="(item,index) in span" :key=index :to="item1[index]" tag="div">
            <a href="#" @click="handleSelect(index)">
                <i :class="[(className[index]),(index==selectIndex?'color':'')]"></i>
                <span :class="index==selectIndex?'color':''">{{item}}</span>
            </a>
        </router-link>
